...
- **A key doesn't turn red although the session is waiting** — red has three sources: (1) the `Notification` hook (terminal CLI only — the VS Code extension never fires it for inline permission dialogs, verified 2026-08); (2) the `PermissionRequest`/`PermissionDenied` hooks (newer Claude Code versions; sessions must be **reloaded once** after `install-hooks.py` so they pick the events up); (3) transcript detection for **questions and plan approvals** (`AskUserQuestion`/`ExitPlanMode`), which needs no hooks at all and works everywhere with ≤ 10 s delay. If permission prompts stay yellow, your Claude Code version predates the permission hooks — questions still turn red via (3).
